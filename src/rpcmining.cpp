...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("block value",    (uint64_t)GetProofOfWorkReward(pindexBest->nHeight, 0, pindexBest->pprev)));
    ...
    obj.push_back(Pair("stake interest",    (uint64_t)MAX_PROOF_OF_STAKE_STABLE));
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "CircuitCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "CircuitCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "CircuitCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "CircuitCoin is downloading blocks...");
